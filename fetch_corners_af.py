...
TARGETS=[292,98,307,188,99,293,323,170]; SEASONS=[2024,2025]
S=requests.Session()
...
def run(budget=38):
    ...
    for af in TARGETS:
        ...
        tp=os.path.join(dd,"_tried.json"); tried=set(json.load(open(tp))) if os.path.exists(tp) else set()
        fixmap={}
        for s in SEASONS:
            fc=os.path.join(dd,f"_fix{s}.json")
        ...
        todo=[f for f in fixmap if f not in done and f not in tried]
        todo=todo[:max(0,250-len(corners))]
        ...
        json.dump(corners,open(cf,"w")); json.dump(list(tried),open(tp,"w"))
